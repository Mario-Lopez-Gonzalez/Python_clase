...
winners = {}
for i,score in enumerate(counts):
    if score > 21:
        if i == 0:
            print(f"El dealer se ha pasado de 21 con un puntuaje de ",score)
        else: 
            print(f"El jugador {i} se ha pasado de 21 con un puntuaje de ",score)
    else:
        if i == 0:
            winners['Dealer'] = score
        else:
            winners[f'Jugador{i}'] = score
winners = dict(sorted(winners.items(), key=lambda item: item[1], reverse=True))
print("Fin del juego, los ganadores en orden son:",winners)